...
static int pmem_from_user(void *dst, const void __user *src, unsigned size)
{
	...
	int rc = __copy_from_user_inatomic_nocache(dst, src, size);

	/*
	 * On x86_64 __copy_from_user_inatomic_nocache uses non-temporal
	 * stores for the bulk of the transfer, but we need to manually
	 * flush if the transfer is unaligned. A cached memory copy is
	 * used when destination or size is not naturally aligned. That
	 * is:
